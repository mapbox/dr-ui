...
function sortVersions(versions) {
  ...
  const latestStable = allVersionsOrdered.filter(version => {
  ...
  // else do nothing
  const sortPreReleases = laterThanLatest.length ? laterThanLatest : latestVersionPreReleases.length ? latestVersionPreReleases : [];
  const newestPreRelease = sortPreReleases && sortPreReleases.sort((a, b) => {
    return compareVersions(a.version, b.version);
  }).reverse().reduce((arr, v) => {
