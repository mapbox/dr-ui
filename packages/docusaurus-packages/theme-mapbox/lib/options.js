...
const defaultPrismTheme = prism_react_renderer_1.themes.github;
...
const DocsSchema = utils_validation_1.Joi.object({
  ...
  sidebar: utils_validation_1.Joi.object({
    hideable: utils_validation_1.Joi.bool().default(
      DEFAULT_DOCS_CONFIG.sidebar.hideable
    ),
    autoCollapseCategories: utils_validation_1.Joi.bool().default(
...
const NavbarItemPosition = utils_validation_1.Joi.string()
  .equal('left', 'right')
  .default('left');
...
const DropdownSubitemSchema = utils_validation_1.Joi.object({
  ...
  switch: [
    ...
    {
      is: utils_validation_1.Joi.alternatives().try(
...
const DocsVersionDropdownNavbarItemSchema = NavbarItemBaseSchema.append({
  ...
  dropdownItemsBefore: utils_validation_1.Joi.array()
    .items(DropdownSubitemSchema)
    .default([]),
  dropdownItemsAfter: utils_validation_1.Joi.array()
    .items(DropdownSubitemSchema)
    .default([])
});
...
const ColorModeSchema = utils_validation_1.Joi.object({
  ...
  disableSwitch: utils_validation_1.Joi.bool().default(
    DEFAULT_COLOR_MODE_CONFIG.disableSwitch
  ),
  respectPrefersColorScheme: utils_validation_1.Joi.bool().default(
...
const LogoSchema = utils_validation_1.Joi.object({
  ...
  width: utils_validation_1.Joi.alternatives().try(
    utils_validation_1.Joi.string(),
    utils_validation_1.Joi.number()
  ),
  height: utils_validation_1.Joi.alternatives().try(
    utils_validation_1.Joi.string(),
    utils_validation_1.Joi.number()
  ),
...
const PrismLanguage = utils_validation_1.Joi.string().custom((val) =>
  val.toLowerCase()
);
exports.ThemeConfigSchema = utils_validation_1.Joi.object({
  ...
  navbar: utils_validation_1.Joi.object({
    ...
    hideOnScroll: utils_validation_1.Joi.bool().default(
      exports.DEFAULT_CONFIG.navbar.hideOnScroll
    ),
    // TODO temporary (@alpha-58)
  ...
  footer: utils_validation_1.Joi.object({
    style: utils_validation_1.Joi.string()
      .equal('dark', 'light')
      .default('light'),
    logo: LogoSchema,
    copyright: utils_validation_1.Joi.string(),
    links: utils_validation_1.Joi.alternatives(
      utils_validation_1.Joi.array().items(
        utils_validation_1.Joi.object({
          title: utils_validation_1.Joi.string().allow(null).default(null),
          items: utils_validation_1.Joi.array()
            .items(FooterLinkItemSchema)
            .default([])
        })
      ),
  ...
  prism: utils_validation_1.Joi.object({
    theme: utils_validation_1.Joi.object({
      plain: utils_validation_1.Joi.alternatives()
        .try(utils_validation_1.Joi.array(), utils_validation_1.Joi.object())
        .required(),
      styles: utils_validation_1.Joi.alternatives()
        .try(utils_validation_1.Joi.array(), utils_validation_1.Joi.object())
        .required()
...
const PluginOptionSchema = utils_validation_1.Joi.object({
  customCss: utils_validation_1.Joi.alternatives()
    .try(
      utils_validation_1.Joi.array().items(
        utils_validation_1.Joi.string().required()
      ),
      utils_validation_1.Joi.alternatives().conditional(
        utils_validation_1.Joi.string().required(),
        {
