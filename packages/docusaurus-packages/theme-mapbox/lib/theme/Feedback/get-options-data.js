...
const LabelItem = ({ title, subTitle, selected, onFeedbackChange }) => {
  ...
  return (
    ...
    >
      <div className="txt-ms">{title}</div>
      {subTitle && <div className="txt-light txt-s">{subTitle}</div>}
      {selected && (
        <FeedbackTextarea
          id={'feedback'}
...
export function getOptionsData({ helpful, categoryType, onFeedbackChange }) {
  ...
  if (helpful) {
    return {
      ...
      options: [
        ...helpfulOptionsRaw.map(({ value, title, subTitle }) => {
          return {
            label: (
              <LabelItem
                title={title}
    ...
    return {
      ...
      options: [
        ...notHelpfulOptionsRaw.map(({ value, title, subTitle }) => {
          return {
            label: (
              <LabelItem
                title={title}
