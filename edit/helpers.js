...
export function extractor(fullHtml) {
  ...
  const js = fullHtml.match(scriptRegex)[1];
  ...
  if (fullHtml.match(srcRegex)) {
    const srcArr = fullHtml.match(srcRegex).map(src => src.replace('src=', '').replace(/["']/g, ''));
  ...
  if (fullHtml.match(hrefRegex)) {
    const hrefArr = fullHtml.match(hrefRegex).map(src => src.replace('href=', '').replace(/["']/g, ''));
